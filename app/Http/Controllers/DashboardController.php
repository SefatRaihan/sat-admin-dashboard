...
class DashboardController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $leadBoard = ExamAttempt::completed()
            ...
            ->with('user.student') // eager load student relation
            ->get()
            ...
            ->map(function ($attempt) {
                ...
                return [
                    'user_id' => $user->id ?? 'N/A',
                    'user_name' => $user->student->name ?? 'N/A',
                    'score' => $percentCorrect,
                    'time' => sprintf("%d:%02d", $minutes, $seconds),
                    'submitted_at' => $attempt->end_time,
                    'profile_image' => $user && $user->student && $user->student->image
                        ? Storage::disk('public')->url($user->student->image)
