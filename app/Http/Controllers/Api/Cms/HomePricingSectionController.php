...
use App\Models\HomePricingSection;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\DB;

class HomePricingSectionController extends Controller
{
    public function index()
    {
        return HomePricingSection::with('items')->latest()->get();
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($validated, $items, &$section) {
            $section = HomePricingSection::first();

            if ($section) {
                $section->update($validated);
                $section->items()->delete();
            } else {
                $section = HomePricingSection::create($validated);
            }
        ...
        return response()->json([
            'message' => 'Home pricing section saved successfully',
