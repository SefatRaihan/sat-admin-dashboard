<x-backend.layouts.student-master>
    <!-- Ensure CSRF token is included in the layout -->
    <meta name="csrf-token" content="{{ csrf_token() }}">

    <div class="mt-3">
        <h4 class="text-center score-title">Your score: <span class="scoreValue">{{ $correctAnswers }}</span></h4>
        <p class="text-center score-text">Your performance is better than <b><span class="scoreValue">{{ $betterThanPercent }}</span>% of <span class="studentName">Mubhir</span> students</b> who have <br> completed this exam</p>
        <div class="mt-4">
    ...
    <div class="mt-4" style="min-height: 100vh;">
        <div class="row">
            <div class="col-md-8">
    ...
    @push('js')
        <script>
            $(document).ready(function() {
                ...
                // Slider input update
                $('#min-range, #max-range').on('input', updateSliderLabels);
            ...
            function resetQuestionFilters() {
                $('#correctAnswerToggle').prop('checked', true);
                $('#incorrectAnswerToggle').prop('checked', true);
            }
            ...
            function updateSliderLabels() {
                const minVal = parseInt($('#min-range').val());
                const maxVal = parseInt($('#max-range').val());
                const minLabel = formatTime(minVal);
                const maxLabel = formatTime(maxVal);
                $('#min-label').text(minLabel);
                $('#max-label').text(maxLabel);
                $('#slider-value').text(`${minLabel} - ${maxLabel}`);
            }
            ...
            function applyFilters() {
                const filters = {
                    ...
                    correct: $('#correctAnswerToggle').is(':checked'),
                    incorrect: $('#incorrectAnswerToggle').is(':checked'),
                    difficulties: $('.difficulty:checked').map(function() {
                        return $(this).val();
                    }).get(),
                    minDuration: parseInt($('#min-range').val()),
                    maxDuration: parseInt($('#max-range').val()),
                ...
                $('#questionTableBody').html('<tr><td colspan="6" class="text-center loading-spinner"><i class="fas fa-spinner fa-spin"></i>Loading...</td></tr>');

                $.ajax({
                    url: '/api/filter-exam-questions',
                    type: 'POST',
                    contentType: 'application/json',
                    data: JSON.stringify(filters),
                    success: function(response) {
                        updateQuestionTable(response.questions);
                    },
                    error: function(xhr) {
                        let message = 'Failed to load questions. Please try again.';
                        if (xhr.status === 422) {
                            message = xhr.responseJSON?.error || message;
                        } else if (xhr.status === 404) {
                            message = 'No questions found for this attempt.';
                        }
                        $('#questionTableBody').html(`<tr><td colspan="6" class="text-center">${message}</td></tr>`);
                    }
                });
            }
                ...
                questions.forEach(item => {
                    ...
                    const row = `
                        <tr>
                            ...
                            <td>${item.question.section || '-'}</td>
                            <td>
                                <span class="badge badge-pill ${difficultyClass}">${item.question.difficulty.toLowerCase()}</span>
                            </td>
            ...
            function fetchOtherStudentScore() {
                ...
                $.ajax({
                    url: '/api/other-student-score',
                    type: 'GET',
                    data: { user_id: userId, exam_id: examId },
                    success: function(response) {
                        $('.other_avg_time').text(response.averagePaceFormatted || '0:00');
                        $('.others-total-time').text(response.totalTimeFormatted || '0:00');
                    },
                    error: function(xhr) {
                        console.error('Leaderboard Error:', xhr.responseText);
                        $('.other_avg_time').text('0:00');
                        $('.others-total-time').text('0:00');
                    }
                });
            }
