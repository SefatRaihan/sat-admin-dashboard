<x-backend.layouts.master>
                    ...
                    <table class="table table-hover align-middle">
                        <thead>
                            <tr align="center">
                                ...
                                <th data-column="avg_time" class="sortable text-left">Best</th>
                                    ...
                                    <div class="row mt-2">
                                        <div class="col-md-12 mt-2">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="exam_name" placeholder="Verbal Section">
                                            </div>
                                        </div>
                                        <div class="col-md-12">
                                            <div class="row">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content" style="border-radius: 24px; height:100%">
                    <div class="modal-header text-left d-flex pb-3">
                        <h5 class="modal-title" id="exampleModalLongTitle"><b>Change Exam Ranking</b></h5>
                    ...
                    <div class="modal-body">
                        <div style="border: 1px solid #D0D5DD; background-color:#F9FAFB; border-radius:8px; padding:12px">
                            <p>Select the ranking you want to reposition the exam. Other exams will be re-ranked accordingly.</p>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('input change', '.no_of_exams, .duration', calculateTotalSectionValues);
        ...
        function fetchExams(page = 1, perPage = 10) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#max-exam-count').text(maxExamCount);
                    ...
                    if (response.data.length === 0) {
                        ...
                        $.each(response.data, function(index, exam) {
                            let statusChecked = exam.status === 'active' ? 'checked' : '';
                            let ranking = exam.ranking || '-';
                            let upDisabled = exam.ranking === 1 ? 'disabled' : '';
                            let downDisabled = exam.ranking === maxExamCount || !exam.ranking ? 'disabled' : '';

                            rows += `<tr>
                                <td style="display: flex; align-items: center; justify-content: center; gap: 15px; height:80px">
                                    <button class="btn btn-link move-ranking" data-id="${exam.id}" data-direction="up" ${upDisabled}>
                                        <i class="fas fa-arrow-up"></i>
                                    </button>
                                    <span style="font-size:16px; font-weight: 600;">${ranking}</span>
                                    <button class="btn btn-link move-ranking" data-id="${exam.id}" data-direction="down" ${downDisabled}>
                                        <i class="fas fa-arrow-down"></i>
                                    </button>
                                </td>
                                <td>${exam.title || ''}</td>
                                <td>${exam.sections[0]?.audience || ''}</td>
                                <td>${exam.section || ''}</td>
                                <td>${exam.total_question_count || 0}<p>${exam.duration || 0}<span>min</span></p></td>
                                <td>${exam.sections[0]?.difficulty || 'N/A'}</td>
                                <td>${exam.avg_time || '00:00'} min</td>
                                <td>N/A</td>
                                <td>${formatDate(exam.created_at)} ${exam.created_by?.full_name || ''}</td>
                                <td>
                                    <label class="switch">
                                ...
                                <td class="text-center">
                                    <button data-toggle="modal" data-target="#detailModalCenter" data-id="${exam.id}" class="btn btn-sm change-btn openDetailModal">Change</button>
                                </td>
                            </tr>`;
                        });
                        tableBody.html(rows);
                ...
                error: function() {
                    Swal.fire('Error', 'Error fetching exams.', 'error');
                }
            });
        }
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '/api/exam-delete',
                        type: 'POST',
                        data: { exams: selectedExams, _token: '{{ csrf_token() }}' },
                        success: function(response) {
        ...
        function show() {
            let examId = $(this).data('id');
            $('.edit-exam').data('id', examId);
            $('#proceedBtn').data('exam-id', examId);
        ...
        function moveRanking() {
            ...
            $.ajax({
                url: `/api/exams/${examId}/move-ranking`,
                type: 'POST',
